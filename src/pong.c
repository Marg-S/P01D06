...
int coordinate_x(int ball_x, int ball_y) {  //смена направления шарика от ракеток
    if (ball_x == 75 && (rocket_r == ball_y - 1 || rocket_r == ball_y || rocket_r == ball_y + 1))
        vector_x = -1;
    if (ball_x == 4 && (rocket_l == ball_y - 1 || rocket_l == ball_y || rocket_l == ball_y + 1)) vector_x = 1;
    if (ball_x == 76 && (rocket_r == ball_y - 1 || rocket_r == ball_y || rocket_r == ball_y + 1))
        vector_x = -1;
    if (ball_x == 3 && (rocket_l == ball_y - 1 || rocket_l == ball_y || rocket_l == ball_y + 1)) vector_x = 1;
